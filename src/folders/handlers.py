...
from folders.schemas import FolderTasksUser, FolderCreate, FolderRead
...
async def get_folder_queries(repo: RequestRepo = Depends(get_repo)) -> SelectSearchResponse:
    folders = await repo.folders.get_all()
    folders_in = [FolderRead.model_validate(folder, from_attributes=True) for folder in folders]
                    ...
                    components=[
                        *[
                            c.Div(
                                components=[
                                    c.Link(components=[
                                        c.Div(components=[c.Text(text=folder.name)], class_name='card-body text-black'),
...
async def get_folder(folder_id: int, repo: RequestRepo = Depends(get_repo)) -> list[
    c.AnyComponent]:
    ...
    return [
        c.Page(
            components=[
                ...
                get_navbar(),
                c.Link(components=[c.Text(text='Add Task')],
                       on_click=GoToEvent(url=f'/jt/tasks/forms/task_form?folder_id={folder_id}'),
                ...
                c.Table(
                    ...
                    columns=[
                        ...
                        DisplayLookup(field='start_time', table_width_percent=15,
                                      mode=DisplayMode.date),
                        DisplayLookup(field='end_time', table_width_percent=15,
                                      mode=DisplayMode.date),
...
@form_router.get("/folder_form", response_model=FastUI, response_model_exclude_none=True)
...
async def create_folder(form: Annotated[FolderCreate, fastui_form(FolderCreate)],
                        repo: RequestRepo = Depends(get_repo)) -> list[c.AnyComponent]:
    folder = await repo.folders.create(form)
    return [
        get_navbar(),
        c.Heading(text='Папка была создана успешно', level=6, class_name='text-success'),
        c.Link(components=[c.Text(text='Перейти к папке')], on_click=GoToEvent(url=f'/jt/folders/{folder.id}'),
