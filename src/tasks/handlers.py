...
from .schemas import TaskCreate, TaskUser, TaskRead
...
async def get_tasks(repo: RequestRepo = Depends(get_repo)):
    ...
    return [
        c.Page(components=[
            ...
            c.Link(components=[c.Text(text='Add Task')], on_click=GoToEvent(url='/jt/tasks/forms/task_form'),
            ...
            c.Table(
                ...
                columns=[
                    ...
                    Display(value='user.username', title='User',
                            on_click=GoToEvent(url='/jt/users/{user.id}')),
                    DisplayLookup(field='start_time', title='Дата начала', mode=DisplayMode.date),
                    DisplayLookup(field='end_time', title='Дата завершения', mode=DisplayMode.date),
                ],
            ),
        ]),
    ]
...
async def task_form(repo: RequestRepo = Depends(get_repo), folder_id: int | None = None):
    initial = {}
    if folder_id:
...
async def get_tasks_queries(repo: RequestRepo = Depends(get_repo)) -> SelectSearchResponse:
...
async def create_task(
    ...
    task_in: TaskCreate = TaskCreate(**form.model_dump(exclude={'folder', 'user'}), folder_id=int(form.folder),
                                     user_id=int(form.user))
    task = await  repo.tasks.create(task_in)
    return [
        get_heading('Карта была создана успешно'),
        get_navbar(),
        c.Link(components=[c.Text(text='Вернуться к карте')], on_click=GoToEvent(url=f'/jt/tasks/{task.id}')),
    ]


@router.get("/{task_id}", response_model=FastUI, response_model_exclude_none=True)
async def get_task(task_id: int, repo: RequestRepo = Depends(get_repo)) -> list[c.AnyComponent]:
        ...
        c.Page(
            components=[
                ...
                c.Link(components=[c.Text(text='Add task')],
                       on_click=GoToEvent(url=f'/jt/tasks/forms/row_form?task_id={task_in.id}'),
                       class_name='btn btn-primary'),
                c.Details(data=task_in, fields=[
                    DisplayLookup(field='title'),
                    DisplayLookup(field='text'),
                    Display(value=task_in.user.username, title='User',
